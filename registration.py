...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs(i):
            if i in memo:
                return memo[i]
            children = self.get_adjacent_vertices(i)
            if not children:
                m = 0
            else:
    ...
    def get_registration_plan(self):
        ...
        courses = []
        in_degree = {index: 0 for index in range(len(self.vertices))}
        ...
        empty = [index for index in in_degree if in_degree[index] == 0]

        while empty:
            empty.sort(key=lambda index: (-self.vertices[index].depth, self.vertices[index].label))
            semester = []
            available = []
